        ...
        <div className="pb-12 md:pb-20">
          ...
          <div className="relative">
            ...
            {items.map((item, index) => (
              <img key={index} className="absolute rounded-full z-10 animate-float" style={item.style} src={require(`../images//user-${index + 1}.jpg`)} width={item.size} height={item.size} alt={`User ${index + 1}`} onMouseEnter={() => { setActive(index); setCommentOn(true); }} onMouseLeave={() => setCommentOn(false)} />
            ))}
